  ...
  <div class="panel panel-default">
    ...
    <div class="panel-body">
      <div class="row">
        ...
        <div class="col-md-12">
          <div class="form-group">
              <label for="artworks-dropzone">Diela</label>
              <div class="needsclick dropzone" id="artworks-dropzone">
                <div class="dz-message needsclick">
                  <strong>Drop</strong> files here or <strong>click</strong> to upload.<br />
                  <span class="note needsclick small text-muted">(Supported file types are images only. Max filesize: 6MB)</span>
                </div>
          ...
          <div class="tab-content top-space">
            ...
            <div role="tabpanel" class="tab-pane  {{ ($i==0) ? 'active' : '' }}" id="{{ $locale }}">
              ...
              <div class="form-group">
                  ...
                  <div class="needsclick dropzone document-dropzone" id="document-dropzone-{{$locale}}" data-locale="{{ $locale }}" >
                    <div class="dz-message needsclick">
                      <strong>Drop</strong> files here or <strong>click</strong> to upload.<br />
...
<script>
  ...
  $(".document-dropzone").each(function() {
  ...
  var uploadedArtworkMap = {};
  $('#artworks-dropzone').dropzone({
    url: '{{ route('authority.storeMedia') }}',
    maxFilesize: 6, // MB
    addRemoveLinks: true,
    headers: {
    ...
    previewTemplate: `
      <div class="dz-preview dz-file-preview">
        ...
        <div class="dz-error-mark">
         ...
         <input type="text" class="form-control input-sm" style="max-width:120px" placeholder="[sk] názov" name="artwork_title_sk[]">
         <input type="text" class="form-control input-sm" style="max-width:120px" placeholder="[sk] datovanie, výtvarný druh" name="artwork_sub_title_sk[]">
         <input type="text" class="form-control input-sm" style="max-width:120px" placeholder="[sk] zdroj" name="artwork_photo_credit_sk[]">
         <input type="text" class="form-control input-sm" style="max-width:120px" placeholder="[en] názov" name="artwork_title_en[]">
         <input type="text" class="form-control input-sm" style="max-width:120px" placeholder="[en] datovanie, výtvarný druh" name="artwork_sub_title_en[]">
         <input type="text" class="form-control input-sm" style="max-width:120px" placeholder="[en] zdroj" name="artwork_photo_credit_en[]">
      </div>`,
    success: function (file, response) {
      $('form').append('<input type="hidden" name="artwork[]" value="' + response.name + '">')
      uploadedArtworkMap[file.name] = response.name
    },
    removedfile: function (file) {
      file.previewElement.remove()
      var name = ''
      if (typeof file.file_name !== 'undefined') {
        ...
        name = uploadedArtworkMap[file.name]
      }
      $('form').find('input[name="artwork[]"][value="' + name + '"]').remove()
    },
    init: function () {
      @if(isset($authority) && $authority->artworks)
        var files = {!! json_encode($artwork_media_files) !!};
        ...
        for (var i in files) {
          ...
          this.options.thumbnail.call(this, file, file.path);
          file.previewElement.classList.add('dz-complete');
          $('form').append('<input type="hidden" name="artwork[]" value="' + file.file_name + '">');
          file.previewElement.querySelector('input[name="artwork_title_sk[]"]').value = file.title_sk;
          file.previewElement.querySelector('input[name="artwork_sub_title_sk[]"]').value = file.sub_title_sk;
          file.previewElement.querySelector('input[name="artwork_photo_credit_sk[]"]').value = file.photo_credit_sk;
          file.previewElement.querySelector('input[name="artwork_title_en[]"]').value = file.title_en;
          file.previewElement.querySelector('input[name="artwork_sub_title_en[]"]').value = file.sub_title_en;
          file.previewElement.querySelector('input[name="artwork_photo_credit_en[]"]').value = file.photo_credit_en;
        }
      @endif
    }
  });
