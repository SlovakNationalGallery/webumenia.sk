...
@section('content')
    <section class="tailwind-rules">
        <filter-new :authors="{{ $authors }}"
            v-slot="{ isExtendedOpen, openedFilter, handleMultiSelectChange, selectedOptionsAsLabels, toggleIsExtendedOpen, handleSortChange, handleCheckboxChange, clearFilterSelection, toggleSelect, closeOpenedFilter, clearAllSelections, query, filters }">
            <div class="tw-bg-gray-200 tw-p-16 md:tw-pb-0">
                <div class="tw-flex tw-gap-x-3 tw-overflow-x-auto md:tw-flex-wrap">
                    <filter-new-custom-select :close-opened-filter="closeOpenedFilter"
                        :opened-filter="openedFilter" :handle-multi-select-change="handleMultiSelectChange" :clear-filter-selection="clearFilterSelection"
                        :filter="filters['authors']" :toggle-select="toggleSelect"
                        :selected-values="query['authors']" name="authors"
                        placeholder="Napíšte meno autora / autorky">
                    </filter-new-custom-select>
                    <filter-new-custom-select :close-opened-filter="closeOpenedFilter"
                        :opened-filter="openedFilter" :handle-multi-select-change="handleMultiSelectChange" :clear-filter-selection="clearFilterSelection"
                        :filter="filters['authors']" :toggle-select="toggleSelect"
                        :selected-values="query['authors']" name="authors" v-if="isExtendedOpen"
                        placeholder="Napíšte meno autora / autorky">
                    </filter-new-custom-select>
                    <filter-new-custom-select :close-opened-filter="closeOpenedFilter"
                        :opened-filter="openedFilter" :handle-multi-select-change="handleMultiSelectChange" :clear-filter-selection="clearFilterSelection"
                ...
                <filter-show-more class="tw-visible tw-pt-4 md:tw-hidden"
                    :is-extended-open="isExtendedOpen" :toggle-is-extended-open="toggleIsExtendedOpen">
                </filter-show-more>
            </div>
            ...
            <div
                ...
                <filter-new-custom-checkbox :handle-checkbox-change="handleCheckboxChange"
                    :checkbox-value="Boolean(query['has_iip'])" title="Len so zoomom" name="has_iip">
                </filter-new-custom-checkbox>
                <filter-new-custom-checkbox :handle-checkbox-change="handleCheckboxChange"
                    :checkbox-value="Boolean(query['is_free'])" title="Len voľné" name="is_free">
                </filter-new-custom-checkbox>
                <filter-new-custom-checkbox :handle-checkbox-change="handleCheckboxChange"
                    :checkbox-value="Boolean(query['has_text'])" title="Len s textom" name="has_text">
                </filter-new-custom-checkbox>
            </div>
            <div class="tw-invisible tw-bg-gray-200 tw-px-16 tw-pb-16 md:tw-visible">
                <filter-new-selected-labels :handle-multi-select-change="handleMultiSelectChange"
