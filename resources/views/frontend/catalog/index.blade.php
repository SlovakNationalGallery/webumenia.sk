    ...
    <section class="catalog" data-searchd-engine="{!! Config::get('app.searchd_id') !!}">
        <div class="container content-section">
            <div class="row content-section">
                <div class="col-xs-6">
                    @if (!empty($search))
                        <h4 class="inline">{{ utrans('katalog.catalog_found_artworks') }} &bdquo;{{ $search }}&ldquo; (<span data-searchd-total-hits>{{ $paginator->total() }}</span>) </h4>
                    @else
                        <h4 class="inline">{{ $paginator->total() }} {{ trans('katalog.catalog_artworks') }} </h4>
                    @endif
