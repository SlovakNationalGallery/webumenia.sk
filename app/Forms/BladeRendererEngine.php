...
//        $sections = $this->resources[$cacheKey][$blockName]->with($variables)->renderSections();
//        return $sections[$blockName];

        $cacheKey = $view->vars[self::CACHE_KEY_VAR];
        $variables['__env'] = new Factory(
            app('view.engine.resolver'),
            new FileViewFinder(new Filesystem(), [
                resource_path('views/form/default')
            ]),
            new Dispatcher()
        );
...
//        }
//
//        $this->resources[$cacheKey][$blockName] = $view->make($theme);
//        return true; // only if section exists
