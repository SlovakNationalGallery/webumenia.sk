...
<body>
            ...
            <section id="painting" class="section-container">
                <div class="container">
                    ...
                    <ul class="grid cs-style-3">
                        <?php $counter = 0; ?>
                        <?php $max = (16 < count($data->response->docs)) ? 16 : count($data->response->docs); ?>
                        <?php $item = array(); ?>

                    	<?php for ($i = 0; $i < $max; $i++) { // ($data->response->docs as $key => $item) { ?>
                        <?php $counter = $i; ?>
                        <!-- dielo-->
                        <?php $item = $data->response->docs[$i] ?>
    ...
    $(window).scroll(function() {
         if($(window).scrollTop() + $(window).height() == $(document).height()) {
            <?php if ($counter+1 < count($data->response->docs)) { $counter++;  ?>
            <?php $item = $data->response->docs[$counter] ?>
            $(".grid").append('<a href="<?php echo getImagePath($item->id, true); ?>"  title="<?php echo $item->firstAuthor .' : '. $item->ti[0] ?>"><li class="painting"><figure><img src="<?php echo getImagePath($item->id); ?>" alt="painting"/><figcaption><h5 class="white"><?php echo $item->firstAuthor ?></h5><span><?php echo $item->ti[0] ?></span></figcaption></figure></li></a>');
